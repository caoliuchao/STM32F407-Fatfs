...
/* for data alignment, the following buffer is prepared for read and write disk,
which is used when the data is not in 4K, otherwise it can be done directly*/
...
SD_Error SD_ReadBlock(uint8_t *readbuff, uint64_t ReadAddr, uint16_t BlockSize)
{
  ...
  {
	  timeout--;   //Wait for transmission to complete 
  }
  if( timeout == 0)
  {
...
/*------------------------------自己添加函数----------------------------------*/
/******************************************************************************
*   函数名：SD_ReadDisk(uint8_t *readbuff, uint32_t sector, uint16_t cnt)
*   功  能：读SD卡
*   输  入：
*          *readbuff：读取数据缓存
*          sector：读取block地址
...
SD_Error SD_ReadDisk(uint8_t *readbuff, uint32_t sector, uint16_t cnt)
{
	...
	if((uint32_t)readbuff%4 != 0)
	{
	 	for(n = 0;n < cnt; n++)
		{
		 	state = SD_ReadBlock(SDIO_DATA_BUFFER, SectorAddr + 512*n, 512);    	//单个sector的读操作
            if(state == SD_OK)
			{
	...
	else
	{
		if(cnt == 1)	
		{
			state = SD_ReadBlock(readbuff, SectorAddr, 512);    	//单个sector的读操作
            if(state == SD_OK)
			{
				/* Check if the Transfer is finished */
				while(SD_GetStatus() != SD_TRANSFER_OK);
		...
		{
			state = SD_ReadMultiBlocks(readbuff, SectorAddr, 512, cnt);//多个sector 
            if(state == SD_OK)
			{
				/* Check if the Transfer is finished */
				state = SD_WaitReadOperation();
				while(SD_GetStatus() != SD_TRANSFER_OK);
			}            
		}
	}
...
/*********************************************************************
*   函数名：SD_WriteDisk(uint8_t *writebuff, uint32_t sector, uint16_t cnt)
*   功  能：写SD卡
*   输  入：
*           *writebuff：读取数据缓存
*           sector：读取block地址
...
SD_Error SD_WriteDisk(uint8_t *writebuff, uint32_t sector, uint16_t cnt)
{
	...
	if((uint32_t)writebuff%4 != 0)
	{
	 	for(n = 0; n < cnt; n++)
		{
			memcpy(SDIO_DATA_BUFFER,writebuff,512);
		 	state = SD_WriteBlock(SDIO_DATA_BUFFER, SectorAddr+512*n, 512);    	//单个sector的写操作
            if(state == SD_OK)
			{
				/* Check if the Transfer is finished */
				while(SD_GetStatus() != SD_TRANSFER_OK);
	...
	else
	{
		if(cnt == 1)	
		{
			state = SD_WriteBlock(writebuff, SectorAddr, 512);    	//单个sector的写操作
            if(state == SD_OK)
			{
				/* Check if the Transfer is finished */
				while(SD_GetStatus() != SD_TRANSFER_OK);
		...
		{
			state = SD_WriteMultiBlocks(writebuff, SectorAddr, 512, cnt);	//多个sector  
            if(state == SD_OK)
			{
				/* Check if the Transfer is finished */
				state = SD_WaitWriteOperation();
				while(SD_GetStatus() != SD_TRANSFER_OK);
			}
		}
	}
